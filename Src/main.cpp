...
{
    srand(time(0));
    //Khởi tạo các chức năng trong thư viện SDL
    if (TTF_Init() != 0) {
    ...
    //load các ảnh games lên texture
    ...
    while (!quit)
    {
        ...
        {
            std::cerr<<"process event type: "<<event.type<<std::endl;
        ...
        if (gameState == MENU) {
            ...
            // Hiển thị high score
            std::string highScoreText = "High Score: " + std::to_string(highScoreManager.getHighScore());
